...
const getGuestsGuestMode = async (authUserId) => {
  try {
  let response = await fetch(`https://27f9-193-61-207-186.eu.ngrok.io/api/user/v1/guests/browse/${authUserId}/guestmode/`);
...
const getGuestsHostMode = async (authUserId) => {
  try {
  let response = await fetch(`https://27f9-193-61-207-186.eu.ngrok.io/api/user/v1/guests/browse/${authUserId}/hostmode/`);
  let json = await response.json();
  setGuests(json)
