...
const getGuest = async (item) => {
  try {
    ...
    let response = await fetch(`https://4c33-193-61-207-186.eu.ngrok.io/api/user/v1/profile/${item.id}/`);
...
const getParty = async (authUserId) => {
  ...
  let response = await fetch(`https://4c33-193-61-207-186.eu.ngrok.io/api/party/v1/myparties/${authUserId}/`);
  let json = await response.json();
  ...
  const sendInvite = async (partyId, guestId) => {
      try {
        ...
        let response = await fetch(
              `https://4c33-193-61-207-186.eu.ngrok.io/api/invite/v1/createinvite/`,
              {
                method: "POST",
                headers: {
